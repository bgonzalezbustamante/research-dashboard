...
import LocationLabelsSection from '@/components/hours/location-labels-section'
...
export default async function WorkSessionsSection({
  date,
  dailyLogExists,
  sessions,
  labels,
  ...
  const locationLabelsAvailable =
    !locationResult.error

  if (
    locationResult.error &&
    locationResult.error.code !==
      'PGRST205'
  ) {
    console.error(
      'Location labels could not be loaded:',
      locationResult.error
    )
  }

  const locationLabels =
    locationLabelsAvailable
      ? ((locationResult.data ??
          []) as LocationLabel[])
      : []
