-- Distant Forge follow-up: add canonical project years and a public Featured flag.
...
alter table public.projects
  add constraint projects_start_year_range
  check (
    start_year is null
    or start_year between 1000 and 9999
  ),
  add constraint projects_end_year_range
  check (
    end_year is null
    or end_year between 1000 and 9999
  ),
  add constraint projects_year_order
  check (
    start_year is null
    or end_year is null
...
drop index if exists public.project_public_metadata_listing_idx;
...
comment on function public.list_public_projects() is
  'Anonymous-safe listing of explicitly Public projects with canonical project URL/years, Featured state, static asset filenames, and associated Public paper slugs only.';

comment on function public.get_public_project(text) is
  'Anonymous-safe slug lookup for an explicitly Public project with canonical project URL/years, Featured state, static asset filenames, and associated Public paper slugs only.';
